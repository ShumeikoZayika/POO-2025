...
public class Livro {
	...
	// Construtor sobrecarregado com título e autor (valores por defeito para os outros)
	...
	public static void main(String[] args) {
		// Criação de instâncias da classe Livro usando vários construtores
		...
		// Criação de uma coleção de 10 instâncias da classe Livro
		Livro[] livros = new Livro[10];
		for (int i = 0; i < livros.length; i++) {
			livros[i] = new Livro("Título " + (i + 1), "Autor " + (i + 1));
		}

		// Exibindo informações dos livros na coleção
		for (int i = 0; i < livros.length; i++) {
